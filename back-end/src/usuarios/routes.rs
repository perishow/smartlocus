...
async fn coletar_usuario_por_id_handler(State(pool): State<MySqlPool>, Path(id): Path<i32>) -> Result<Json<Usuario>, (StatusCode, String)> {
    let usuario_service = UsuariosService::new(pool);
    match usuario_service.coletar_usuario_por_id(id).await {
        Ok(usuario) => Ok(Json(usuario)),
        Err(e) => Err(((StatusCode::NOT_FOUND), "Usuário não encontrado".to_string())),
    }
}
...
async fn login_handler(
    ...
    let usuario_service = UsuariosService::new(pool);
    match usuario_service.validar_usuario_e_senha(payload.nome, payload.senha).await {
        Ok(usuario) => Ok(Json(usuario)),
        Err(mensagem) => {
